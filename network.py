...
class Network:
    ...
    execute_siggen(self, device_id): Simulates a signal generator and updates 
                                    its outputvsignal value.

    update_siggens(self): If it is time to do so, sets siggen signals to RISING
                         or FALLING.                         
    ...
    def update_siggens(self):
        ...
        for device_id in siggen_devices:
            ...
            if device.siggen_waveform[device.siggen_counter] == "0":
                device.outputs[None] = self.devices.FALLING
            elif device.siggen_waveform[device.siggen_counter] == "1":
                device.outputs[None] = self.devices.RISING
